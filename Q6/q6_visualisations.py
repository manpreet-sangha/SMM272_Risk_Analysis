...
def fig_q6_2_risk_decomposition(
    ...
    ax = axes[0]
    bars1 = ax.bar(x - width/2, comp_var, width, color=colors,     alpha=0.85, label="Component VaR")
    bars2 = ax.bar(x + width/2, comp_es,  width, color=[C_ES]*4,   alpha=0.75, label="Component ES",
                   edgecolor="white")

    ax.axhline(0, color="black", linewidth=0.8, linestyle="-")
    ax.axhline(var,  color=C_VAR, linewidth=1.2, linestyle="--", alpha=0.6,
               label=f"Portfolio VaR = ${var:,.0f}")
    ax.axhline(es,   color=C_ES,  linewidth=1.2, linestyle=":",  alpha=0.6,
               label=f"Portfolio ES  = ${es:,.0f}")

    ax.set_xticks(x)
    ax.set_xticklabels([TICKER_LABELS[t] for t in tickers], fontsize=8.5)
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"${v:,.0f}"))
    ax.set_ylabel("USD", fontsize=10)
    ax.set_title("Component VaR and Component ES\n(Euler allocation, non-Gaussian)", fontsize=11)
    ...
    ax2.set_xticks(x)
    ax2.set_xticklabels([TICKER_LABELS[t] for t in tickers], fontsize=8.5)
    ax2.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"${v:,.0f}"))
    ax2.set_ylabel("USD per contract", fontsize=10)
    ax2.set_title("Marginal VaR and Marginal ES\n(per additional long contract)", fontsize=11)
    ax2.legend(fontsize=8.5, framealpha=0.9)
    ax2.grid(axis="y", alpha=0.3)

    fig.suptitle("99% Risk Decomposition — Options Portfolio", fontsize=13, y=1.01)
    fig.tight_layout()
    return _save(fig, "fig_q6_2_risk_decomposition.png")
...
def fig_q6_3_correlation_heatmap(
    ...
    for ax, corr, title in zip(
        axes,
        [corr_ewma, corr_hist],
        ["EWMA Correlation (λ=0.94)", "Historical Correlation (2-year)"],
    ):
    ...
    fig.suptitle("Stock Return Correlation Matrices", fontsize=13)
    fig.tight_layout()
    return _save(fig, "fig_q6_3_correlation_heatmap.png")
...
def fig_q6_4_option_profiles(
    ...
    for j, (leg, ax) in enumerate(zip(portfolio, axes_flat)):
        S0    = leg["spot"]
        K     = leg["strike"]
        T0    = leg["ttm"]
        ...
        pct_moves = np.linspace(-0.30, 0.30, 200)
        ...
        ax.plot(pct_moves * 100, pnl_pos, color=color, linewidth=2.0, label="Position P&L")
        ax.axhline(0,  color="black",    linewidth=0.8, linestyle="-")
        ax.axvline(0,  color="grey",     linewidth=0.8, linestyle="--", alpha=0.6)
        ax.axvline((K/S0 - 1) * 100, color="black", linewidth=1.2,
                   linestyle=":", label=f"Strike K=${K:.2f}")

        # Shade profit / loss regions
        ax.fill_between(pct_moves * 100, pnl_pos, 0,
                        where=(pnl_pos >= 0), alpha=0.2, color="green")
        ax.fill_between(pct_moves * 100, pnl_pos, 0,
                        where=(pnl_pos < 0),  alpha=0.2, color="red")

        ax.set_title(f"{leg['ticker']} — {leg['description']}", fontsize=9.5)
        ax.set_xlabel("Underlying Price Change (%)", fontsize=9)
        ax.set_ylabel("Position P&L (USD)", fontsize=9)
        ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"${v:,.0f}"))
        ax.legend(fontsize=8, framealpha=0.9)
        ax.grid(alpha=0.3)

    fig.suptitle("Option Position P&L at 10-Day Horizon\n"
                 "(Full revaluation using Black-Scholes, historical volatility)",
                 fontsize=12)
    fig.tight_layout()
    return _save(fig, "fig_q6_4_option_profiles.png")
...
def fig_q6_5_simulated_returns(
    ...
    for j, (ticker, ax) in enumerate(zip(tickers, axes_flat)):
        ...
        # Simulated histogram (normalised)
        ax.hist(sim_r, bins=80, density=True, color=color, alpha=0.55,
                label="Simulated 10-day returns")
        ...
        ax.plot(x_range, norm.pdf(x_range, mu, std), color="black",
                linewidth=1.6, linestyle="--", label="Normal fit")
        ...
        ax.axvline(np.percentile(sim_r,  1), color=C_VAR, linewidth=1.4,
                   linestyle=":", label="VaR 1% quantile")

        ax.set_title(f"{TICKER_LABELS[ticker]}", fontsize=9.5)
        ax.set_xlabel("10-day Log-return", fontsize=9)
        ax.set_ylabel("Density", fontsize=9)
        ax.legend(fontsize=7.5, framealpha=0.9)
        ax.grid(alpha=0.3)

    fig.suptitle("Simulated 10-Day Stock Log-Return Distributions\n"
                 f"({config.Q6_N_SIMS:,} simulations, EWMA covariance, λ=0.94)",
                 fontsize=12)
    fig.tight_layout()
    return _save(fig, "fig_q6_5_simulated_returns.png")
